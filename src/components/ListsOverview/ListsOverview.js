...
import style from "./ListsOverview.module.css";
  ...
  return (
    <div className={style.scrollable}>
      ...
      <section className={style.topOverview}>
        <h1>
          Rimanenti <span>{`(${mainList.length})`}</span>
        </h1>
        <button
      ...
      {/* Lista dei completati */}
      <section className={style.topOverview}>
        <h1>
          Completati <span>{`(${rolledList.length})`}</span>
        </h1>
        <button
          onClick={() =>
            setShowDone((prev) => ({
      ...
      <section className={style.topOverview}>
        <h1>
          Errori <span>{`(${mistakesList.length})`}</span>
        </h1>
        <button
          onClick={() =>
            setShowMistakes((prev) => ({
